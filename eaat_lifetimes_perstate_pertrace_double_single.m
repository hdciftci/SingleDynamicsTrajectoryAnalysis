function varargout = eaat_lifetimes_perstate_pertrace_double_single( filename, options )
...
for trace=1:numel(dwells),
  ...
  %keep int states separate !!!
        ...
        if state==2 
          ...
          dwellc=dwells{trace}(find(dwells{trace}(:,1)==state),2)*sampling;
        end
