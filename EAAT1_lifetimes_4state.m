function varargout = EAAT1_lifetimes_4state( filename, options )
% meantime nStates gives NAN instead of 0!!!!!
% loadDwelltimes  Combined list of dwell-times for each state.]
% this function assumes you have a 4-state model idealization. All
% variations of IFS is considered to be the same state and their dwell
% times are concatanated, intermediate state is grouped with either OFS or
% IFS
%
...
for trace=1:numel(dwells),
        % change extended state 1 (0 in dwt files) ID to 2 unless it is the
        %last state(photobleaching)
        dwells{trace}(:,2)
        if size(dwells{trace},1)>2 && dwells{trace}(end-1,2)==1
            dwells{trace}(end-1,:)=[];
        end
        dwells{trace}(:,2)
        for i=1:size (dwells{trace},1)-1
         if dwells{trace}(i,1)==1   
            dwells{trace}(i,1)=2;
         end
        end
        % merging states 1 and 2 (0 and 1 in dwt) since we will consider
        % them to be one state only
        k=size(dwells{trace},1);
        l=2;
        while l<=k
            if dwells{trace}(l-1,1)==2 && dwells{trace}(l,1)==2
                dwells{trace}(l-1,2)=dwells{trace}(l-1,2)+dwells{trace}(l,2);
                dwells{trace}(l,:)=[]; 
                l=l-1;
            end
            k=size(dwells{trace},1);
  ...
  %keep int states separate !!!
        ...
        %skip intermediate states we'll put them together with either OFS
        %(state 4 (or state 3 dwt)) or IFS (state 2&1 (or state 1 and 0 in dwt))
        if state>=3 && state<nStates
            continue;
        end
        % gathering all visits to IFS in one vector
        if state==2 % since ifs/ofs and ofs/ofs will have ofs dwell time information in it you should analyze them together
            dwellc=dwells{trace}(find(dwells{trace}(:,1)==state),2)*sampling; % here we are fetching the visits of one trace to a particular state in seconds, we converted this remember!!! 
            for i=2:size (dwells{trace},1)
                if dwells{trace}(i,1)>2 && dwells{trace}(i,1)<nStates
                    if dwells{trace}(i-1,1)>dwells{trace}(i,1) 
...
for trace=1:nTraces,
    for state=2:nStates,
        ...
        if isempty(y)==1 % this is when there is no histogram created for that state and trace; could be due to two reasons either there are no visits to that state or the number of visits is less than 4 so a meaningful histogram cannot be created.
       ...
       % if coeffs(3)/coeffs(4)<3 && coeffs(3)/coeffs(4)>=1 || coeffs(3)/coeffs(4)>0.3 && coeffs(3)/coeffs(4)<=1
        ...
        if isnan(lifetimes.(fieldname)(trace,4))==0 
            if lifetimes.(fieldname)(trace,4) >=0.15 && fit_type==2
               ...
               lifetimes.(fieldname)(trace,3) = coeffs2(1,1)*(-1/coeffs2(1,3))+(1-coeffs2(1,1))*(-1/coeffs2(1,4));
